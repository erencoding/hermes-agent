...
class WebSearchProvider(ABC):
    ...
    ``web_tools._SEARCH_PROVIDERS``.  The user selects a provider via
...
class WebExtractProvider(ABC):
    ...
    Implementations live in sibling modules and are registered in
    ``web_tools._EXTRACT_PROVIDERS``.  The user selects a provider via
    ``hermes tools``; the choice is persisted as
    ...
    Extract providers return results in a normalized format::

        {
            "success": True,
            "data": [
                {"url": str, "title": str, "content": str, "metadata": dict},
                ...
            ]
        }
